...
uint256 hashGenesisBlock("0x04dad2dada7490bc8a6184274339d66aff9c816c0ac1d89695635cbecc2cbeb1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // fckbankscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "fckbankscoin Signed Message:\n";
...
int64 nMinimumInputValue = DUST_HARD_LIMIT / 100;
...
{
    // fckbankscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 730000); // fckbankscoin: 730k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60 * 2; // Every 2 hours
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
  ...
  if (LatestBlockTime < BlockReading->GetBlockTime()) {
    if (BlockReading->nHeight > 125000) {
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 60000) { DiffMode = 2; }
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // fckbankscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // fckbankscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
	...
	pchMessageStart[0] = 0xfb;
        ...
        hashGenesisBlock = uint256("0x6ca3cf39c1a2a68a7e6cbd8baa19f8e5671591851bb3ad6d115f79fce22dcdb4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Building a river to flow people from the banks...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1388290935;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1210004;

        if (fTestNet)
        {
	    block.nTime    = 1388287628;
            block.nNonce   = 576945;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xbc7d555022013f1186eb2aa53145214d6170919b3e3bfbe0c4d39d37a4a7f2ed"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("fckbankscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("fckbankscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static fckbankscoinMiner(CWallet *pwallet)
{
    printf("fckbankscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fckbankscoin-miner");
    ...
    try { loop {
        ...
        printf("Running fckbankscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("fckbankscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&fckbankscoinMiner, pwallet));
}
